...
fn main(
    ...
    let r = vec2<f32>(0.0);// vec2(random(uvec3(index, 0)), random(uvec3(index, 1)));
    ...
    let r3 = vec3<f32>(0.0); //vec3(random(uvec3(index, 2)), 
                //    random(uvec3(index, 3)), 
                //    random(uvec3(index, 4)));

    let angle = 0.0; //random(uvec3(index, 5)) * 2.0 * 3.14159265;

    let position = read_texture(DISPLACEMENTS_LAYER, global_id);
    ...
    grass_storage.entries[ubo.storage_base_entry + entry][i].albedo = ((canopy.rgb - 0.5) * 0.025 + albedo_value + vec3<f32>(-.0)) * mix(vec3<f32>(.5), vec3<f32>(1.5), r3);
