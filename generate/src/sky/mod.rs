...
pub(crate) trait LookupTableDefinition: Sync {
    ...
    fn generate<F: FnMut(String, usize, usize) + Send>(
        &self,
        mut progress_callback: F,
    ) -> Result<LookupTable, Error> {
        let size = self.size();
        let total = size[0] as u64 * size[1] as u64 * size[2] as u64;
        progress_callback(format!("Generating {}... ", &self.name()), 0, total as usize / 1000);
